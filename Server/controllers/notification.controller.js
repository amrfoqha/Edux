...
module.exports.findNotificationsByUserId = async (req, res) => {
  try {
    const answer = await Notification.find({
...
module.exports.findNotification = async (req, res) => {
  try {
    const answer = await Notification.findOne({ _id: req.params.id }).populate(
      "user"
    );
    res.json(answer);
  } catch (error) {
...
module.exports.createNotification = async (req, res) => {
  try {
    ...
    const populated = await answer.populate("user");
...
module.exports.updateNotification = async (req, res) => {
  try {
    const resp = await Notification.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    }).populate("user");
    res.json(resp);
